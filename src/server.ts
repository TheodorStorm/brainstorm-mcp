...
export class AgentCoopServer {
  ...
  /**
   * Cached working directory set by the status tool (session-scoped state).
   * Automatically cleared when MCP server process terminates with Claude Code session.
   * No explicit cleanup needed - process memory is freed by OS.
   */
  private cachedWorkingDirectory?: string;
  ...
  /**
   * Get the cached working directory or return an error if not initialized.
   *
   * This helper ensures that the session has been properly initialized by calling
   * the `status` tool first. The working directory is cached for the lifetime of
   * the MCP server process (one Claude Code session).
   *
   * @returns Cached working directory string or error response
   *
   * @example
   * const workingDirResult = this.getCachedWorkingDirectory();
   * if ('error' in workingDirResult) {
   *   return workingDirResult; // Return error response
   * }
   * const workingDirectory = workingDirResult;
   */
  private getCachedWorkingDirectory(): string | { content: { type: string; text: string }[]; isError: true } {
    if (!this.cachedWorkingDirectory) {
      return {
        content: [{
          type: 'text',
          text: JSON.stringify({
            error: 'Session not initialized. Please call the \'status\' tool first with working_directory parameter to initialize your Brainstorm session.',
            code: 'SESSION_NOT_INITIALIZED',
            hint: 'The status tool should be called automatically on Claude Code startup via your shell wrapper.'
          })
        }],
        isError: true
      };
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      const tools: Tool[] = [
        ...
        {
          name: 'join_project',
          description: 'Join a project with agent name. Session must be initialized by calling status tool first.',
          inputSchema: {
            type: 'object',
            properties: {
              project_id: {
        ...
        {
          name: 'leave_project',
          description: 'Leave a project gracefully. Archives your unread messages and removes your membership. You can rejoin later to reclaim your agent name. Session must be initialized by calling status tool first.',
          inputSchema: {
            type: 'object',
            properties: {
              project_id: {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case 'create_project': {
            // Get cached working directory (or return error if not initialized)
            const workingDirectoryResult = this.getCachedWorkingDirectory();
            if (typeof workingDirectoryResult !== 'string') {
              return workingDirectoryResult;
            }
            const workingDirectory = workingDirectoryResult;
            ...
            // Auto-join creator as coordinator member
            if (metadata.created_by) {
              const clientId = resolveClientId(workingDirectory);
              const member: ProjectMember = {
                project_id: metadata.project_id,
                agent_name: metadata.created_by,
                agent_id: randomUUID(),
                client_id: clientId,
              ...
              await this.storage.auditLog({
                ...
                details: { agent_id: member.agent_id, client_id: clientId, auto_joined: true }
              });
            }
              ...
              content: [{
                ...
                text: JSON.stringify({
                  ...
                  message: metadata.created_by
                    ? `Project created and you (${metadata.created_by}) have been added as coordinator.`
                    : 'Project created. Agents can now join using join_project.'
                }, null, 2)
              }]
            };
          }
          ...
          case 'join_project': {
            ...
            const workingDirectoryResult = this.getCachedWorkingDirectory();
            if (typeof workingDirectoryResult !== 'string') {
              return workingDirectoryResult; // Return the error response
            }
            const workingDirectory = workingDirectoryResult;
          ...
          case 'status': {
            ...
            // Cache the working directory for use in all other tools (session-scoped state)
            this.cachedWorkingDirectory = workingDirectory;
